			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="name" title="${message(code: 'poem.name.label', default: 'Name')}" />
				...
				<g:each in="${poemInstanceList}" status="i" var="poemInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${poemInstance.id}">${fieldValue(bean: poemInstance, field: "name")}</g:link></td>
